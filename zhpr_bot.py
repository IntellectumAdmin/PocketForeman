...
ЖПР-бот (Журнал производства работ) v5
...
- 🔁 Быстрое добавление ещё одного вида работ на том же участке
  через кнопку "➕ Ещё вид работ (этот же участок)"
...
async def _create_notion_page_from_context(
    ...
    # Техника (тип) — Multi-select в Notion
...
async def btn_add_same_entry(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Старт новой записи ЖПР на том же участке, что и последняя сохранённая:
    - та же дата
    - тот же раздел (ГПР)
    - тот же подраздел / участок
    ...
    if not template:
        # Если нет шаблона — ведём как обычную новую запись
...
async def st_equip_type(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt in ("-", "—", ""):
        ...
        # В Notion это мульти-селект, поэтому все значения сохраняем в список
...
async def st_review(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt == BTN_SAVE:
        # Сохраняем текущий контекст как шаблон для "ещё вид работ"
        current = context.user_data.get("zhpr") or {}
        # В шаблоне НЕ держим план/факт/вид работ/комментарий — они будут новые
        ...
        if ok:
            ...
            await update.message.reply_text(
                ...
                reply_markup=ReplyKeyboardMarkup(
                    [[BTN_NEW_ENTRY]], resize_keyboard=True
                ),
            )
        context.user_data.pop("zhpr", None)
        context.user_data.pop("zhpr_struct", None)
...
async def on_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Ловим нажатие кнопки "Новая запись ЖПР" или
    "Ещё вид работ (этот же участок)" вне диалога.
    """
    txt = (update.message.text or "").strip()
