...
def _kb_for_parent(parent_path: str) -> InlineKeyboardMarkup:
    ...
    if parent_path:
        ...
        ctrl.append(
            InlineKeyboardButton("⬅️ Назад", callback_data=f"b|{_id_for_path(parent_parent)}")
        )
    ctrl.append(
        InlineKeyboardButton("✅ Выбрать здесь", callback_data=f"c|{_id_for_path(parent_path)}")
    )
    rows.append(ctrl)
...
async def photo_pick_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act in ("p", "b"):
        text = (
            f"Раздел: {format_path_for_notion(path) if path else 'Корень'}\n"
            f"Выбери подраздел:"
        )
...
async def ph2_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        reply_markup=ReplyKeyboardMarkup(
            [[KeyboardButton(BTN_CHANGE), KeyboardButton(BTN_CANCEL)]],
            resize_keyboard=True,
        ),
    )
...
async def ph3_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not photo_bytes or not section_path:
        await update.message.reply_text(
            "Сессия потеряна. Начни заново: /photo",
            reply_markup=main_menu(),
        )
        return ConversationHandler.END
    ...
    try:
        ...
        await update.message.reply_text(
            f"✗ Ошибка загрузки в Cloudinary: {e}",
            reply_markup=main_menu(),
        )
        return ConversationHandler.END
...
async def on_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not url:
        ...
        return
    if _seen_recent(url):
        await msg.reply_text("✓ Принято (повтор).", reply_markup=main_menu())
        return
...
async def on_text_buttons(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # 1) Старт / Добавить фото -> сразу в выбор раздела
    if txt in (BTN_START, BTN_ADD_PHOTO):
        context.user_data.clear()
    ...
    # Иное — просто игнор (чтоб не плодить лишние сообщения)
    return
...
def main():
    ...
    try:
        info = sync_structure()
        log.info(
            f"✓ Структура синхронизирована при старте. "
            f"Корень: {info['root']}, разделов: {len(info['paths'])}"
        )
    except Exception as e:
