...
- SafeSync структуры
- /photo выбор раздела и загрузка
- Камера через WebApp (camera.html) + приём web_app_data
- Фолбэк: фото из галереи (берём первый кадр, если медиагруппа)
"""
...
def _notion_create_row_once(section: str, file_name: str, url: str, comment: Optional[str]) -> Tuple[bool, str]:
    ...
    try:
        j = r.json(); return False, str(j.get("message") or j.get("error") or j)
    except Exception:
        return False, r.text
...
async def photo_pick_cb(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if act == "c":
        ...
        cam_url = (
            ...
            f"&cam=back"
            f"&save=1"           # авто-сохранение на телефон
            f"&v={ts}"
        )
        ...
        # Один аккуратный inline-фолбэк (если клавиатура снизу не поднимает WebApp)
        kb_open_inline = InlineKeyboardMarkup(
            [[InlineKeyboardButton("📷 Открыть камеру (если снизу не работает)", web_app=WebAppInfo(cam_url))]]
        )
        await query.message.reply_text("Если кнопка снизу не открывает встроенное окно — нажми эту:",
                                       reply_markup=kb_open_inline)

        # Фото из галереи (можно альбомом — возьмём первый кадр)
...
async def ph3_comment(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not photo_bytes or not section_path:
        await update.message.reply_text("Данные потерялись. Начни заново: /photo", reply_markup=main_menu())
        return ConversationHandler.END
    ...
    if ok:
        await update.message.reply_text("✓ Фото загружено в Cloudinary и добавлено в Notion.",
                                        reply_markup=ReplyKeyboardRemove())
        # Дублируем изображение в чат — чтобы Telegram мог сохранить его в Галерею (настройка «Сохранять в галерею»)
        try:
...
async def on_webapp_data(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if ok:
        # шлём картинку в чат — так Telegram при желании сохранит её в галерею
...
def main():
    ...
    app.add_handler(CallbackQueryHandler(photo_pick_cb, pattern=r"^(p|b|c)\|"))  # запасной

    # Команды
    app.add_handler(CommandHandler("start", cmd_start))
    app.add_handler(CommandHandler("sync", cmd_sync))

    # Прочие текстовые кнопки
